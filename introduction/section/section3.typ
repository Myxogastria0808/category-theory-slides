...
- $t$$dots.c.h$終点関数
  ...
  - $s(a) = v$および$t(a) = w$であるような$a in "Arrow"$が与えられたとき、$a$は$v$から$w$への辺である。この有向グラフは、$v ->^a w$のような図式で表現できる。
- Gの道 (path) $dots.c.h$それぞれの辺の終点が次の辺の始点であるような辺の列。
  - 道には、$G$の辺$a in A$にすぎない長さ$1$の列や、同じ頂点を始点および終点として、どの辺もたどらない長さ$0$の列も含まれる。
- 事実: 任意の有向グラフから擬順序が得られる。

== 自由圏

任意の有向グラフ$G = ("Vertex", "Arrow", s, t)$に対して、
...
#block(
  ...
  有向グラフをそのまま圏に変換することができると理解しておけば十分である。
]

== 自由圏の例 #h(0.5em) $cal("Free")(bold(2))$の場合

#slide[
  有向グラフ$c_1 ->^(f_1) c_2$によって生成される自由圏$cal("Free")(bold(2))$を以下のように定義する。

  $
    cal("Free")(bold(2)) := cal("Free")(v_1 ->^(f_1) v_2)
  $
...
#slide[
  ...
  4. $forall c, d, e in cal("Ob")(cal(C))$と射$f in cal(C)(c, d), g in cal(C)(d, e)$に対して、$f$と$g$の合成射$f;g in cal(C)(c, e)$を規定
    - 合成は道の結合より与えられる。
...
#slide[
  ...
  #v(1em)
  従って、$cal("Free")(v_1 ->^(f_1) v_2)$は圏である。
]
...
#slide[
  *自由圏*\
  自由圏 (有限表示をもたない圏) は、例えば以下の図式のように$f;h$と$g;i$が等しい有限表示をもたない圏である。
...
#slide[
  圏$cal("Set")$は、集合と関数をそれぞれ対象と射に対応させた圏である。
  #v(1em)
  圏であるためには、以下の6つの制約を満たす必要があるので、満たしているかどうかを確認する。\
  1. 対象の集まり$cal("Ob")(cal(C))$を規定
...
#slide[
  ...
  6. 結合律$dots.h.c$$forall f: c_0 -> c_1, g: c_1 -> c_2, h: c_2 ->c_3$に対して、$(f;g);h = f;(g;h)$が成立する。この合成を単に$f;g;h$と表現する。
    - $f : S -> T, g : T -> U, h : U -> V$について、$((f;g);h)(s)=h((f;g)(s)) = h(g(f(s)))$ (ただし、任意の$s in S$) である。また、$(f;(g;h))(s)=(g;h)(f(s))=h(g(f(s)))$ (ただし、任意の$s in S$) である。任意の$s in S$で値が等しいので、関数も等しい。従って、結合律の条件を満たす。
  #v(1em)
...
#slide[
  圏$cal(C)$上の$cal(I)$-型の図式とは、共変関手$F: cal(I) -> cal(C)$や$G: cal(I) -> cal(C)$のこと。\
  この圏$cal(I)$のことをこれらの図式の添字圏とよぶ。\

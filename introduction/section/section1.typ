...
発表を始めるにあたって、数学の世界でよく使われる言い回しを紹介する。 \
以降の説明の中で、これらの言い回しが出てきた場合は、以下の意味で使われていると理解して欲しい。\

- `well-defined`$dots.c.h$意味、値、対象が一意に定まる
...
#slide[
  ...
  4. $forall c, d, e in cal("Ob")(cal(C))$と射$f in cal(C)(c, d), g in cal(C)(d, e)$に対して、$f$と$g$の合成射$f;g in cal(C)(c, e)$を規定
    - $c in cal("Ob")(cal(C))$と$c in cal(C)$を等価な表現とする \
    - $f in cal(C)(c, d)$と$f: c -> d$を等価な表現とする \
]
...
#text(
  0.8em,
)[なお、以降の図式では、$arrow.cw$の記号は省略することが多い。]

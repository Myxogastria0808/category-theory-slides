...
- 対象について
  - 共変関手$F: cal(C) -> cal(D)$は、圏$cal(C)$の任意の対象$c$を圏$cal(D)$のある対象$F(c)$に写す。
- 射について
  - 共変関手$F: cal(C) -> cal(D)$は、圏$cal(C)$の任意の射$f: c-> d$を圏$cal(D)$のある射$F(f): F(c) -> F(d)$に写し、以下を満たす。
...
- 自然性条件
  - 任意のコンポーネント$alpha_c$と$alpha_d$と圏$cal(C)$の任意の射$f: c -> d$に対して、次スライドの図式が可換であることを要求する。

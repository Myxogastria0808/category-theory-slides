...
#slide[
  ...
  以下に有効グラフの反例と制約を示す。
  - Tableのidを参照している列については ● 、それ以外を参照している列については ○ として各列の型に向けてエッジを伸ばしている。
  - 各列には必ずただ一つの型が与えられる。
]
...
4.2のスライドで示した通り、有効グラフ$C$と$D$は、自由圏に変換することができる。\
従って、有効グラフ$C, D$は、それぞれ自由圏$cal(C), cal(D)$に変換することができる。
...
等式制約の表現方法は、以下の通りである。
- 表現方法は、通過したノード または エッジを`.`で連結して記述する。
- e.g. $N_1 ->^(E_1) N_2$ ($N_1$から出発して$E_1$を経由し、$N_2$に到達した場合): $N_1.E_1$

自由圏$cal(C)$の有限表示は、以下の通りである。
- $"Employee"."manager"."dept" = "Employee"."dept"$ (自由圏$cal(C)$における等式制約)

自由圏$cal(C)$の有限表示の制約を自由圏$cal(D)$に写した際の有限表示は、以下の通りである。
- $"Staff"."supervisor"."team" = "Staff"."team"$ (自由圏$cal(D)$における等式制約)
